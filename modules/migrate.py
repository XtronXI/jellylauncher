...
def metadata(context):
    ...
    try:
        ...
        items = conn.execute("""
            SELECT Id, Type, Path FROM BaseItems
        ...
        for item in items:
            ...
            old_guid = str(jellyfin_guid(item["Type"], other_path, context.data_dir, case_sensitive=context.case_sensitive)).upper()
            if old_guid == stored:
                continue
        ...
        if path_updates or folder_moves or missing:
            msg = f"Updated {path_updates} metadata paths, moved {folder_moves} folders"
            if missing:
                msg += f", {missing} items had no stored metadata"
            ui.verbose(context, msg + ".")
        else:
            ui.verbose(context, "Metadata already up-to-date.")
        ui.success("Migrating metadata")
    except Exception as error:
        ui.fail("Migrating metadata")
        raise MigrationError(f"Metadata migration failed: {error}") from error
...
def mblink(context):
    ...
    try:
        ...
        raise MigrationError(f".mblink migration failed: {error}") from error
            ...
            for _, column, col_type, *_ in cur.fetchall():
                ...
                for rowid, value in rows:
                    if not isinstance(value, str):
                        continue
                    new_value = _translate_data_json(value, context) if table == "BaseItems" and column == "Data" else translate(value, context)
                    if new_value == value:
                        continue
                    cur.execute(f'UPDATE "{table}" SET "{column}" = ? WHERE rowid = ?', (new_value, rowid))
                    updates += 1
        conn.commit()
        conn.close()
...
def guids(context, db=None):
    ...
    try:
        ...
        try:
            ...
            for row in rows:
                ...
                if new_path == row["Path"]:
                    continue
                new_guid = str(jellyfin_guid(row["Type"], new_path, context.data_dir, case_sensitive=context.case_sensitive)).upper()
        ...
        raise MigrationError(f"GUID migraton failed: {error}") from error
    ...
    for table in tables:
        ...
        for column in columns:
            ...
            if table == "BaseItems" and column_name == "Id":
                continue
            rows = conn.execute(f'SELECT rowid, "{column_name}" FROM "{table}" WHERE "{column_name}" IS NOT NULL').fetchall()
            for rowid, value in rows:
                if not isinstance(value, str):
                    continue
                ...
                if new_value == value:
                    continue
                conn.execute(f'UPDATE "{table}" SET "{column_name}" = ? WHERE rowid = ?', (new_value, rowid))
                updates += 1
    return updates
...
def _rewrite_baseitem_ids(conn, mapping):
    ...
    for old, new in full.items():
        conn.execute("UPDATE BaseItems SET Id = ? WHERE Id = ?", (new, "PARK:" + old))

def _recompute_presentation_keys(conn, mapping):
    changed_ids = set(mapping.values())
    rows = conn.execute("""
        SELECT Id, Type, SeriesId, IndexNumber, PresentationUniqueKey, SeriesPresentationUniqueKey
        FROM BaseItems
    """).fetchall()
    ...
    for row in rows:
        ...
        series_puk = id_to_puk.get(series_id) if series_id is not None else None

        if row["PresentationUniqueKey"] != puk or row["SeriesPresentationUniqueKey"] != series_puk:
            conn.execute(
                "UPDATE BaseItems SET PresentationUniqueKey = ?, SeriesPresentationUniqueKey = ? WHERE Id = ?",
                (puk, series_puk, guid),
            )
            updates += 1
    return updates
...
def xml(context):
    ...
    try:
        ...
        for xml_file in context.data_dir.rglob("*.xml"):
            ...
            file_updates = _translate(tree.getroot(), context) + _deduplicate_options(tree)
            if file_updates:
        ...
        raise MigrationError(f"XML Migration failed: {error}") from error
...
def _deduplicate_options(tree):
    updates = 0
    path_infos = tree.getroot().find("PathInfos")
