import itertools, re, shutil, threading, time
import readchar
...
def show_header():
    ...
    for line in ascii_title.splitlines():
        ...
        if visible.strip():
            print(" " * max(0, (width - len(visible)) // 2) + line)
        else:
            print()
    print()
    print("JellyLauncher v2.1.1  •  © 2026 XtronXI".center(width))
    print()
...
def show_status(context):
    ...
    table.add_row("Case Sensitive", "[green]Yes[/green]" if context.case_sensitive else "[yellow]No[/yellow]")
    panel = Panel(table, title="[bold cyan] SYSTEM [/bold cyan]", border_style="cyan", padding=(1, 2), expand=False)
    console.print(panel, justify="center")

def show_error(error):
    console.print()
    console.print(
        Panel(f"[bold red]Error:[/bold red] {error}", border_style="red", padding=(1, 2), expand=False),
        justify="center",
    )
...
def _render():
    ...
    for name, status in _steps:
        ...
        line.append(name, style="dim" if status == "pending" else "white")
        lines.append(line)
    ...
    lines.append(Text())
    lines.append(Text(f"Elapsed: {elapsed:.1f}s", style="dim", justify="centre"))
    if _completed:
        lines.append(Text())
        lines.append(Text("✓ SUCCESS", style="bold green", justify="center"))

    return Panel(Group(*lines), title=_panel_title, border_style="cyan", padding=(1, 3), expand=False)

def _refresh():
    if _live is not None:
        _live.update(Align.center(_render()))
...
def begin(steps, title="[bold cyan] MIGRATION [/bold cyan]"):
    ...
    _live = Live(refresh_per_second=10, transient=False, get_renderable=lambda: Align.center(_render()))
    _live.start()
